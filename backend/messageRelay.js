...
const { fetchUnseenMessage, markMessageAsSent, markMessageAsFailed } = require('./dao/outboxDao');
...
const pollAndPublish = async () => {
    ...
    try{
        ...
        const published = channel.sendToQueue(QUEUE_NAME, Buffer.from(JSON.stringify(message.payload)), {
            persistent: true
        })

        if (published){
            await markMessageAsSent(message.id);
            console.log(`Message ${message.id} sent successfully and marked as SENT.`);
        } else{
            await markMessageAsFailed(message.id);
            console.error(`Failed to publish message ${message.id}. RabbitMQ buffer full.`);
        }
    } catch (err){
